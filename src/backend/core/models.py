...
class DocumentQuerySet(MP_NodeQuerySet):
    ...
    def time_filter(self, lower_time_bound=None, upper_time_bound=None):
        """
        Filter documents by update/deletion timestamp.

        Args:
            lower_time_bound (datetime, optional):
                Keep documents updated or deleted after this timestamp.
            upper_time_bound (datetime, optional):
                Keep documents updated or deleted before this timestamp.
        ...
        if lower_time_bound and upper_time_bound:
            conditions = (
                ...
                | models.Q(
                    deleted_at__gte=lower_time_bound, deleted_at__lte=upper_time_bound
                )
                | models.Q(
            ...
            conditions = (
                models.Q(updated_at__gte=lower_time_bound)
                | models.Q(deleted_at__gte=lower_time_bound)
                | models.Q(ancestors_deleted_at__gte=lower_time_bound)
            )
        elif upper_time_bound:
            conditions = (
                models.Q(updated_at__lte=upper_time_bound)
                | models.Q(deleted_at__lte=upper_time_bound)
                | models.Q(ancestors_deleted_at__lte=upper_time_bound)
            )
